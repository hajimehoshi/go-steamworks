...
func processZip(dir string) error {
	...
	for path, filename := range map[string]string{
		...
		windows := strings.HasSuffix(filename, ".dll")
		...
		var hashout hash.Hash
		if windows {
			hashout = sha256.New()
			out = io.MultiWriter(out, hashout)
		}
		...
		if windows {
			hash := fmt.Sprintf("%x", hashout.Sum(nil))
			if err := writeWindows(filename, hash); err != nil {
				return err
			}
		}
	}
...
const windowsTmpl = `// SPDX-License-Identifier: Apache-2.0
// Copyright 2021 The go-steamworks Authors
...
const steamAPIDLLHash = "{{.Hash}}"
`

func writeWindows(dllname, hash string) error {
	var filename string
	switch dllname {
	case "steam_api.dll":
		filename = "steamworks_windows_386.go"
	case "steam_api64.dll":
		filename = "steamworks_windows_amd64.go"
	}
	...
	t := template.Must(template.New(filename).Parse(windowsTmpl))
